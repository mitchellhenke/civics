defmodule CivicsWeb.CoreComponents do
  @moduledoc """
  ...
  @doc ~S"""
  Same as `table/1`, but requires `row_id` and uses `:key` so items moving
  between positions don't re-send their content. Not compatible with streams.

  ## Examples

      <.keyed_table id="users" row_id={&"#{&1.id}"} rows={@users}>
        <:col :let={user} label="id">{user.id}</:col>
        <:col :let={user} label="username">{user.username}</:col>
      </.keyed_table>
  """
  ...
  def item_list(assigns) do
    ~H"""
    <ul class={["list-disc list-inside pl-4", @class]}>
  ...
  def translate_error({msg, opts}) do
    ...
    #   Gettext.dngettext(CivicsWeb.Gettext, "errors", msg, msg, count, opts)
    # else
    #   Gettext.dgettext(CivicsWeb.Gettext, "errors", msg, opts)
    # end
